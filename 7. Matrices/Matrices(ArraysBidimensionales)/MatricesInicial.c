/*
 ...
 MARICES O ARRAYS BIDIMENSIONALES
 --------------------------------
 ...
 - Es una variable muy grande que tiene dos dimensiones: Filas y Columnas, que contienen un conjunto de variables
   pequeñas ordenadas en forma consecutiva cuyo contenido pertenece a un tipo de dato homogéneo.

   Cada elemento se identifica por su posición de fila y posición de columna dentro de la matriz.
...
--------------
  ...
  int matriz [2][2];//Matriz de 2X2= 4 elementos.
...
- índice de columna: Es un int >=0 y <cantidad de columnas.
...
--------------
 ...
 El nombre o identificador de la mtriz es una referencia a la primera dirección de memoria
 de la zona de memoria reservada para la misma.
 "EL IDENTIFICADOR DE LA MATRIZ ES UN PUNTERO A LA PRIMERA DIRECCIÓN DE LA ZONA
 ...
 Se realiza a través de la estructura de iteración definida: Dos ciclos for anidados (uno para recorrer una
 dimensión y otro para recorrer la otra dirección. También se puede recorrer con iteración condicionada: Dos ciclos while
 anidados o do-while.
